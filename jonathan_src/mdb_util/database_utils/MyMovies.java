...
public class MyMovies implements MovieDB{
  ...
  public ArrayList<Review> getReviewsFromDatabase(){
  ...
  */
    ...
    try {
      ...
      while (rs.next()) {
        ...
        Review review = new Review(rs.getString("ReviewText"), rs.getInt("Grade"),
                                    rs.getInt("AccountID"), movie_id, rs.getInt("ReviewID"));
  ...
  public ArrayList<CastListing> getCastForMovie(int id_nr){
    ArrayList<CastListing> cast = new ArrayList<CastListing>();
    ResultSet rs = db.executeQuery("SELECT m.MovieID, a.Name, a.DateOfBirth, a.Birthplace, a.ActorID, c.CharacterName FROM actors a, movies m, casts c WHERE m.MovieID = c.MovieID AND a.ActorID = c.ActorID");
    try {
      CastListing cl = null;
      Actors a = null;
      while (rs.next()) {
        if (rs.getInt("m.MovieID")==id_nr) {
          a = new Actors(rs.getString("a.Name"), rs.getString("a.Birthplace"), rs.getString("a.DateOfBirth"), rs.getInt("a.ActorID"));
          cl = new CastListing(a, rs.getString("c.CharacterName"));
          cast.add(cl);
        }
      }
      return cast;
  ...
  public void updateMovieProductionCompany(Movies m, String new_production_company){
    int movie_id = m.id_nr();
    String SQL="UPDATE movies SET ProductionCompany='"+new_production_company+
      "' WHERE MovieID="+movie_id;
    if (db.executeUpdate(SQL) > 1) {
...
public void updateMovieAddActorToCast(int movie_id, int actor_id, String character_name){
  String SQL = "INSERT INTO casts VALUES ("+movie_id+", "+actor_id+", '"+character_name+"')";
  if (db.executeUpdate(SQL) > 1) {
    System.out.println("Error when changing adding actor to cast");
    db.rollback();
  }
  else {
    System.out.println("Actor added to movie's cast");
    db.commit();
  }
}
  public void updateMovieDeleteActorFromCast(Movies m, Actors a, String character_name){
    int movie_id = m.id_nr();
    int actor_id = a.id_nr();
    String SQL = "DELETE FROM casts WHERE MovieID="+movie_id+" AND ActorID="+actor_id+" AND CharacterName='"+character_name+"'";
    if (db.executeUpdate(SQL) > 1) {
      System.out.println("Error when deleting actor from cast");
      db.rollback();
    }
    else {
      System.out.println("Deleted actor from movie's cast");
      db.commit();
    }
  }
  /*
  ...
  */
  public void addMovieToDatabase(Movies m){
    ...
    String production_company = m.production_company();
    int year = m.year();
    int id_nr = m.id_nr();
  ...
  */
  ...
  /*
  *
  * Metod för att hämta standard-kontot
  *
  */
  //(String username, String password, String email, boolean admin_status, int id_nr)
  public Account getStandardAccount(){
    ResultSet rs = db.executeQuery("SELECT * FROM accounts WHERE AccountID=0");
    try {
      Account standard_account = null;
      while (rs.next()) {
        standard_account = new Account(rs.getString("Username"),
         rs.getString("Password"), rs.getString("Email"), rs.getBoolean("AdminStatus"), rs.getInt("AccountID"));
      }
      db.closeIt(rs);
      return standard_account;
    }catch (Exception e) {
      System.err.println("Error getting standard account: " + e.getMessage());
    }
    return null;
  }
}
